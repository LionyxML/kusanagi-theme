...
If you get =Unable to find theme file for 'kusanagi'=, the ~add-to-list~
above is the fix. If you get =Cannot load kusanagi-theme=, your package
archives are not refreshed and the ~modus-themes~ dependency cannot
resolve: run ~M-x package-refresh-contents~ once, then re-evaluate.
